...
import { downloadFileFromURL } from '../../utils/email'


export const postSignRequestEvent = (req: Request, res: Response) => {
  const event = req.body
  if (event.event_type === 'signed') return handleSignedEvent(event, res)
...
const handleSignedEvent = async (event, res) => {
  ...
  let appealFormUrl
  try {
    appealFormUrl = await downloadFileFromURL(event.document.pdf, `forms/${application.id}/appeal_form.pdf`)
  } catch (err) {
