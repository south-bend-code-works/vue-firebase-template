...
export const postApplicationAssessorsEmail = async (req: Request, res: Response) => {
  ...
  if (!(application && form)) return res.status(400).send({message: 'Must include valid application and form.'})

  let attachment
  try {
    attachment = await getFirebaseFileBase64(`forms/${application.id}/appeal_form.pdf`)
  } catch (err) {
