...
export const postApplicationPayment = async (req: Request, res: Response) => {
  ...
  if (!(application && token && origin)) return res.status(400).send({message: 'Must include valid application and token.'})
  if (application.paid) return res.status(400).send({message: 'This has already been paid. No need to pay again!', code: 'ALREADY_PAID'})
  ...
  const adminsToEmail = await getNotifiees('applicant_paid')
  ...
  try {
    await Promise.all(emailsOptions.map(emailOptions => send(emailOptions)))
  } catch (err) {
    console.error(err)
